...
GENETIC_PROFILES = {
    ...
    "APOE 4/4": {
        "amyloid_production": 1.0,       
        "amyloid_clearance": 0.3,        
        "chemotaxis_efficiency": 0.4,    
...
def build_spatial_hippocampal_graph(model, rng):
    ...
    def connect_layers(source_layer, target_layer, connection_prob, long_range_prob):
        ...
        for src in sources:
            ...
            # Optimization: Only look at a random subset of potential targets to speed up init
            ...
            for tgt in potential_targets:
                ...
                # Probability: Sigmoid decay based on distance (Neighbors prefer neighbors)
...
class Neuron(FixedAgent):
    def __init__(self, model, cell, node_id, layer):
        ...
        # Resilience Variance
        self.resilience = self.random.normalvariate(1.0, 0.2) 
        
        # --- COGNITIVE RESERVE ---
        self.reserve = 1.0 * self.resilience 
        
        # --- TAU / STRESS ACCUMULATION ---
        # "Memory" of stress. Even if inputs return, Tau persists.
    ...
    def step(self):
        ...
        for agent in self.cell.agents:
            if isinstance(agent, Microglia):
                if agent.state != "dysfunctional": has_shield = True
                if agent.state == "M1": has_toxic_neighbor = True
                break
        
        shield_strength = 0.8 if (has_shield and local_amyloid < 1.0) else 0.0
        
        # --- 2. DAMAGE INPUTS ---
        dam_amyloid = local_amyloid * (self.model.amyloid_toxicity * 0.0005) * (1.0 - shield_strength)
        ...
        # --- 3. EXPONENTIAL NETWORK STRESS (Avalanche Logic) ---
        network_stress = 0.0
        ...
        if self.model.synapse_graph.has_node(self.node_id):
            ...
            if preds:
                ...
                # Exponential stress: Missing 50% signal = 25% stress. Missing 100% = Massive stress.
                network_stress = ((1.0 - avg_signal) ** 2) * 2.0 

        total_damage = (dam_amyloid + dam_cytokine + dam_proximity + network_stress) * self.vulnerability
        
        # --- 4. DYNAMIC REPAIR & METABOLIC CLIFF (SIGMOID REVISION) ---
        base_repair = self.model.neuron_repair_rate
        
        # REVISION: Logistic Decay instead of Binary Cliff
        # Metabolic efficiency stays high (~1.0) until reserve drops near 0.25, then plummets.
        # This creates the "Slippery Slope" rather than a hard step.
        metabolic_efficiency = 1.0 / (1.0 + np.exp(10.0 * (0.25 - self.reserve)))
        
        # REVISION: Tau Toxicity
        # As stress accumulates, it forms Tau, which physically blocks repair mechanisms.
        self.internal_tau += network_stress * 0.05
        tau_toxicity = min(0.9, self.internal_tau) # Cap at 90% inhibition
        ...
        if self.reserve > 0:
            ...
            # Reserve is gone. Damage hits health directly. No repair possible.
            self.health -= total_damage
            
        # --- 5. DETERMINISTIC DEATH ---
...
class Microglia(CellAgent):
    ...
    def step(self):
        ...
        if self.state == "M0" and local_amyloid > activation_thresh:
            ...
            if self.random.random() < 0.3: self.state = "M0"

        # --- M1 BEHAVIOR & BURNOUT ---
        if self.state == "M1":
            self.model.cytokine[x, y] += 0.05 
...
class ADModel(mesa.Model):
    def __init__(
        self, width=40, height=40, n_neurons=1000, n_microglia=60, # UPDATED SCALE
        genotype="APOE3 (Control)", 
        genetic_overrides=None,  
        amyloid_toxicity=7.9675,    
        cytokine_toxicity=11.0877,
        lecanemab_dose=0.0, 
        cromolyn_dose=0.0,
        neuron_death_threshold=0.3229, 
        neuron_repair_rate=0.015,
        plaque_feedback_rate=0.15,
        microglia_burnout_time=62,
        max_steps=1200, seed=None
    ):
        ...
        if genetic_overrides: base_profile.update(genetic_overrides)
        self.genetic_profile = base_profile

        self.plaque_feedback_rate = plaque_feedback_rate * self.genetic_profile['aggregation_rate']
        ...
        self.lecanemab_efficacy_multiplier = 1.0 + (lecanemab_dose / 5.0) 
        self.cromolyn_dose = cromolyn_dose
        self.cromolyn_efficacy_multiplier = 1.0 + (cromolyn_dose / 50.0)

        self.amyloid = np.zeros((width, height))
        self.cytokine = np.zeros((width, height))

        # --- REORDERED INIT FOR SPATIAL GRAPH ---
        
        # 1. Place Neurons First
        self.neuron_map = {}
        n_DG = int(n_neurons * 0.4)
        n_CA3 = int(n_neurons * 0.3)
        n_CA1 = int(n_neurons * 0.3)
        ...
        # 2. Build Spatial Graph (Now that they have positions)
    ...
    def step(self):
        ...
        self.amyloid += (self.amyloid ** 2.0) * self.plaque_feedback_rate
        ...
        for cell in self.grid.all_cells:
            ...
            if len(neighbors) > 0:
                ...
                for n in neighbors: diffused_cytokine[n.coordinate] += per_neighbor
...
model_params = {
    ...
    # Updated Slider to allow for the new 1000 default
    "n_neurons": { "type": "SliderInt", "value": 1000, "label": "Neurons", "min": 200, "max": 2000, "step": 100 },
}

AD_model_instance = ADModel(width=40, height=40, n_neurons=1000, n_microglia=60)
