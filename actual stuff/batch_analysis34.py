...
# --- IMPORT YOUR MODEL ---
# Ensure 'try_again.py' is in the same directory as this script
try:
...
# Clinical Target Parameters (APOE 3/4 Benchmark)
# Extracted from GENETIC_PROFILES in try_again.py
TARGET_PARAMS = [25.6069, 0.1778, 20.8447]

# Visualization Settings
Y_LIMIT = 30              # Increased slightly as APOE 3/4 might have different variances
DPI = 300                 # High resolution
COLORS = ['#66c2a5', '#fc8d62', '#8da0cb', '#e78ac3', '#a6d854'] # Pastel palette
OUTPUT_DIR = "APOE34_Calibration_Final"
...
def run_single_simulation(seed):
    ...
    model = ADModel(
        ...
        genotype="APOE 3/4",  # <--- CHANGED TO APOE 3/4
        lecanemab_dose=0.0,
        cromolyn_dose=0.0,
...
def run_batch(batch_id):
    ...
    # R2 Calculation
    # Note: If the curve is very flat (low atrophy), R2 can be unstable.
...
if __name__ == "__main__":
    print(f"🚀 STARTING PRODUCTION RUN (APOE 3/4)")
    ...
    for b in range(NUM_BATCHES):
        ...
        # Expanded range slightly for APOE 3/4 as it may fit differently
        if -10.0 < res['r2'] < 0.999:
            results.append(res)
    ...
    for i, res in enumerate(top_5):
        ...
        ax.plot(time_axis, target_curve, color="#444444", linestyle="--", 
                linewidth=2.5, alpha=0.6, label="APOE 3/4 Target")

        # 2. FIT A CURVE to the Simulation Mean
        try:
            # Updated initial guess for APOE 3/4 target params
            p0 = [25, 0.17, 20] 
        ...
        # 4. Formatting
        title_text = f"APOE 3/4 Calibration: Rank {i+1} (Seed {res['master_seed']})"
        ...
        # Explicit 2-Year Intervals on X-Axis
        ...
        # Save
        filename = f"{OUTPUT_DIR}/APOE34_Rank_{i+1}_Seed_{res['master_seed']}.png"
    ...
    df_data.to_csv(f"{OUTPUT_DIR}/APOE34_Top_5_Data.csv", index=False)
