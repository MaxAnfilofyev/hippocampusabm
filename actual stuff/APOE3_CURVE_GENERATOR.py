...
# --- 1. CONFIGURATION ---
NUM_BATCHES = 20          # Total Groups to test
...
# L=100.0, k=0.2587, t0=27.72
# TARGET_PARAMS = [100.0, 0.2587, 27.72]
TARGET_PARAMS = [25.6069, 0.1778, 20.8447]

# Visualization Settings
Y_LIMIT = 30              # Max Y Value (APOE3 usually hits ~12% by Year 20)
...
def run_single_simulation(seed):
    ...
    model = ADModel(
        ...
        genotype="APOE 3/4",  # <--- CHANGED TO APOE3
        lecanemab_dose=0.0,
        cromolyn_dose=0.0,
...
if __name__ == "__main__":
    print(f"🚀 STARTING PRODUCTION RUN (APOE3 Control)")
    ...
    for i, res in enumerate(top_5):
        ...
        ax.plot(time_axis, target_curve, color="#444444", linestyle="--", 
                linewidth=2.5, alpha=0.6, label="APOE3/4 Target")

        # 2. FIT A CURVE to the Simulation Mean
        try:
            # Initial guess adjusted for APOE3 [L=100, k=0.2, t0=25]
            p0 = [100, 0.2, 25] 
        ...
        # 4. Formatting
        title_text = f"APOE3/4 Calibration: Rank {i+1} (Seed {res['master_seed']})"
        ...
        # Save
        filename = f"{OUTPUT_DIR}/APOE3/4_Rank_{i+1}_Seed_{res['master_seed']}.png"
    ...
    df_data.to_csv(f"{OUTPUT_DIR}/APOE3/4_Top_5_Data.csv", index=False)
